...
//        }
        ...
        public function do_delete_collect(){
		 ...
		 public function do_buy(){
            $uid = $this->session->userdata('id');
            $wid = $this->input->get('id');
            $row=$this->user_model->do_buy($wid,$uid);
            if($row){
                echo "<script>alert('购买成功');</script>";
                header("Refresh:0;url=shopuser_personal");
            }else{
			...
			if($uid){
				if($pcont){
					...
					if($rs){
						...
						header("Refresh:0;url=show_single?id=$id");
					}
				}else{
		...
		public function shopuser_personal(){
			$uid = $this->session->userdata('id');
			$rs=$this->user_model->shopuser_personal($uid);
			foreach ($rs as $key1) {
				$key1->aaa=$this->user_model->do_shopuser_personal($key1->wid);	
			}
			$arr['goods']=$rs;
			
		    $this->load->view("shopuser_personal.php",$arr);
		}
		 public function do_delete_collect1(){
            $uid = $this->session->userdata('id');
            $wid = $this->input->get('id');
            $row=$this->user_model->delete_collect1($wid,$uid);
            if($row){
                echo "<script>alert('删除成功');</script>";
                header("Refresh:0;url=shopuser_personal");
            }else{
