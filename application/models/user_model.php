...
//            die;
        ...
        public function get_allrows($id){
		 ...
		 public function do_buy($wid,$uid){
            $arr=array(
                'wid'=>$wid,
                'uid'=>$uid
            );
            $query = $this->db->insert('buy',$arr);
            return $query;
        }
        ...
        public function shopuser_personal($uid){
        	$query=$this->db->get_where('buy',array("uid"=>$uid));
			return $query->result();
        }
		public function do_shopuser_personal($wid){
			$query=$this->db->get_where('goods',array("wid"=>$wid));
			return $query->result();
		}
		 public function delete_collect1($wid,$uid){
            $arr=array(
                'wid'=>$wid,
                'uid'=>$uid
            );
            $query = $this->db->delete('buy',$arr);
            return $query;
        }
